...
from src.services.favorites import FavoriteManager, get_favorite_manager
...
@check_access_token
@inject
def get_all(user: AuthUser, favorite_manager: Annotated[FavoriteManager, Depends(get_favorite_manager)]):
    favorites = favorite_manager.find_all(user_id=user.id)
...
@check_access_token
@inject
def create(user: AuthUser, film_id: UUID, favorite_manager: Annotated[FavoriteManager, Depends(get_favorite_manager)]):
    try:
        ...
        favorite = favorite_manager.create(data_model.model_dump())
...
@check_access_token
@inject
def get(user: AuthUser, film_id: UUID, favorite_manager: Annotated[FavoriteManager, Depends(get_favorite_manager)]):
    favorite = favorite_manager.find_one({"user_id": user.id, "film_id": str(film_id)})
...
@check_access_token
@inject
def delete(user: AuthUser, film_id: UUID, favorite_manager: Annotated[FavoriteManager, Depends(get_favorite_manager)]):
    favorite = favorite_manager.find_one({"user_id": user.id, "film_id": str(film_id)})

    if favorite is None:
        abort(HTTPStatus.NOT_FOUND, description="Film not found")

    favorite_manager.delete(favorite)
